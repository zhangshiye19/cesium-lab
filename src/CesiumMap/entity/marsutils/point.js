...
import { getOnLinePointByLen } from "./matrix";
...
export function getCurrentMousePosition(scene, position, noPickEntity) {
  ...
  try {
    ...
    marslog.log("scene.pick 拾取时异常", e);
  }
    ...
    if (pcEntity) {
      if (pcEntity.show) {
        ...
        if (cartesian) {
          ...
          marslog.log("拾取到被排除的noPickEntity模型", noPickEntity);
        }
      }
    } else {
...
export var windingPoint = {
  ...
  start: function(viewer, point) {
    ...
    this.viewer.mars.centerPoint(this.position, {
...
export function sliceByMaxDistance(positions, maxDistance) {
  ...
  for (var i = positions.length - 1; i >= 1; i--) {
    ...
    var distance = Cesium.Cartesian3.distance(pt1, pt2);
    maxDistance -= distance;

    if (maxDistance == 0) return positions.slice(i);
    else if (maxDistance < 0) {
      maxDistance += distance;
      //求指定长度拼接上去
      var newpt = getOnLinePointByLen(pt1, pt2, maxDistance);
