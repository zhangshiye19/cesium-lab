...
export function getPopup(cfg, attr, options) {
  ...
  if (isArray(cfg)) {
    ...
    let countsok = 0;
    let inhtml = title
      ? '<div class="mars-popup-titile">' + title + "</div>"
    ...
    for (let i = 0; i < cfg.length; i++) {
      ...
      if (thisfield.type == "details" || thisfield.type == "button") {
        ...
        inhtml += `<div style="text-align: center;padding: 2px 0;">
                            ...
                            data-type="${col}" class="btn btn-primary btn-sm mars3d-popup-btn" >${thisfield.name ||
          "查看详情"}</button>
                        </div>`;
        continue;
      ...
      if (thisfield.format) {
        ...
        try {
          ...
          marslog.log("getPopupByConfig方法format格式化异常", thisfield);
        }
      }
        ...
        switch (thisfield.type) {
          ...
          case "number":
            showval = `<input type="${thisfield.type ||
              "text"}" class="mars3d-popup-edititem" style="width:${options.width || 190}px;"
                        data-type="${col}" value="${showval}" placeholder="请输入 ${
              thisfield.name
            }"  />`;
            break;
          case "textarea":
            showval = `<textarea class="mars3d-popup-edititem" style="width:${options.width ||
              190}px;"
    ...
    //全部显示
    let countsok = 0;
    let inhtml = title ? '<div class="mars-popup-titile">' + title + "</div>" : "";
    inhtml += '<div class="mars-popup-content" >';
    for (let col in attr) {
      try {
        if (col == null || attr[col] == null) continue;
        ...
        marslog.log("getPopup方法all构造时异常", e);
      }
    }
    inhtml += "</div>";
...
//对mars3d内置图层的绑定处理（图层一般因为有属性读取及格式化处理）
